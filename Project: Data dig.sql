...
select avg(intelligence) as Avergae_Intelligence
From marvels;

select name, Max(intelligence) as Max_Intelligence
From marvels;
...
select name, Min(intelligence) as Min_Intelligence
From marvels;

select name, hometown, Max(speed) as Max_Speed 
from marvels
where Speed > 1
group by speed
having max(speed) > 5;

select name, popularity, gender, strength,
    case when strength > 5 THEN "Strongest_Avenger"
    Else "Weak"
    End as Strength_Factor
from marvels
order by strength;

select name, height_m, intelligence
from marvels
where height_M < 1.8 and intelligence > 2
